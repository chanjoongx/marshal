...
headroom is well below B15's 10800 W. The operator then types a plain-language note into the single
free-text field. On the live take, use a note that names the rack only by description, `the rack
running the checkpoint writer has a firmware update in 10 min`: the model reads the live rack and job
...
Engineer taps `Override` and types a plain-language note into the single free-text field, something
the telemetry cannot know, and on the live take one that names the rack only by description:
- override note (typed): `the rack running the checkpoint writer has a firmware update in 10 min`
Marshal has the model read the live rack and job list, resolve "checkpoint writer" to the `ckpt-9`
job on B3, and interpret the sentence into a structured constraint, `exclude_rack B3`. A regex could
pull an id out of "B3 has a firmware update" but cannot resolve a description, so this is the beat
where the model is provably load-bearing. Code then checks B3 is a real rack before trusting it
(falling back to a regex parse of the note if the model's does not validate). The id-bearing note
`B3 has a firmware update in 10 min` is the safe fallback if the live model stumbles.
...
- MOCK=0, `CRUSOE_API_KEY` in `.dev.vars`, both models warmed (`npm run probe` green). Optionally
  ...
  co-location, and `node scripts/probe_constraint.mjs` to confirm it still resolves the
  checkpoint-writer description to `exclude_rack B3` (5/5, 2 description-only).
