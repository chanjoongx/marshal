...
live state and real Nemotron output; the prompt constrains the format, so wording will be
close, and the learned chip, timeline, and control flow are deterministic code).
...
Record scenario S1 at 4x. Recording starts at sim t~=88s (a few seconds before the surge).
Each video second is 4 sim-seconds. Without action B7 does not cross 84C until sim t~=460
(video ~93s), well past the clip, and we intervene by sim ~230 (video ~35s), so there is
comfortable margin and B7 never actually throttles on screen.
...
| 45-55s | 268-308 | second event, routes around B12 |
...
- action line on the card: `Migrate job-4471 to B12 and cap B7 intake`
The card is the hero. The heatmap only shows B7 warming in context.
...
- override reason: `B12 is in maintenance, do not target it`  (kind exclude_rack, target B12)
...
- action line: `Migrate job-4471 to B15, move only low-priority jobs, cap B7`
- learned chip on the card: `learned: avoids maintenance racks`
Engineer taps `Approve`. B15 lights up in the heatmap. B7's projected-temp line visibly bends
down and away from the 84C throttle line (projection falls toward ~67C).

### 45-55s  Second event, learning proven
A different aisle spikes: A-row (A4-A6) takes a surge. The new advisory carries the same
`learned: avoids maintenance racks` chip and routes around B12 on its own, without being told
again:
- headline: `A5 projected to hit 84C throttle, routing around B12`
- action line: `Migrate job-4820 from A5 to B14, avoid B12`
Engineer taps `Approve`.

### 55-60s  Resolution
Temps return to nominal. A resolution card shows a mini incident timeline: surge -> warn ->
why -> override -> re-solve -> approve -> second event -> resolved. Always visible at the
bottom: the `SIMULATED TELEMETRY` badge and the footer
...
- Speed set to 4x; window at 1920x1080; hide bookmarks/notifications.
- Dry-run the click path once (Why -> Override B12 -> Approve -> second event -> Approve).
