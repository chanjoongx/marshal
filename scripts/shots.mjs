// Capture live (MOCK=0) product screenshots against a running Marshal for the README, showing the
// real "Nemotron · Xs" chip and a description-only override the model resolves. Defaults to the
// deployed Worker. Run: `SHOT_DIR=docs/img node scripts/shots.mjs` (URL=... to point elsewhere).
...
try {
  ...
  await page.screenshot({ path: DIR + "/advisory.png" }); // full hero
  await page.getByTestId("advisory-card").screenshot({ path: DIR + "/rule-contrast.png" }); // card close-up
  ...
  await page.locator("div.override").screenshot({ path: DIR + "/override.png" }); // the override panel close-up
  ...
  await page.getByTestId("advisory-card").screenshot({ path: DIR + "/resolve.png" }); // the re-solved card close-up
  console.log("OK");
} catch (e) {
